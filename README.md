...
I have always been fascinated by how [React-PDF](https://react-pdf.org/) makes it extremely easy to create PDF files (kudos to @diegomura and the authors) without first creating HTML + CSS and then converting that to a PDF file with some huge web browser architecture. React-PDF has awesome page-wrapping functionality and simple support for fonts. For me, it has been absolutely the most convenient way to create custom reports and various PDFs for my software.
...
```toml
[dependencies]
flexpdf = { version = "0.1.5", default-features = false, features = ["tls-rustls"] }
```
